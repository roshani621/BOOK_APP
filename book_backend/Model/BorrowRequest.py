...
def BorrowBook():
    pipeline = ([
        ...
        },
        {
            ...
            "approved_by": "$approved_details.username",
            "due_date": "$due_date"
            }
        }
        ]
...
def BorrowRequest():
    ...
    record = borrow_request_col.find_one({"id":id})
    borrow_days = record.get("borrow_days", 0)
    ...
    if request_type == 'ACCEPT' and borrow_days <= 0:
        return jsonify({
            "success": False,
            "message": "Borrow days must be greater than 0"
        }), 400

    if request_type == 'ACCEPT' and borrow_days >= 0:
        approved_date = datetime.utcnow()
        due_date = approved_date + timedelta(days=borrow_days)

        update_fields["approved_date"] = approved_date
...
@borrow_request_api.route('/borrow-books', methods=['GET'])
def BorrowRequestBook():
    pipeline = ([
        {
            "$lookup": {
            "from": "Users",
